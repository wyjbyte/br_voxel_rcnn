...
class VoxelRCNNHead(RoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        self.init_weights()
        # 添加PnP3D模块
        self.use_pnp3d = self.model_cfg.get('USE_PNP3D', False)#todo: 这个地方True表示使用PnP3D模块
        if self.use_pnp3d:
            from ...models.pnp3d.pnp3d_module import PnP3D
            pnp3d_cfg = self.model_cfg.PNP3D_CONFIG
            self.pnp3d_module = PnP3D(
                in_channels=c_out,  # 使用池化后的特征通道数
                k=pnp3d_cfg.get('K', 16),  # KNN搜索的邻居数量
                conv_cfg=pnp3d_cfg.get('CONV_CFG', None)
            )
    ...
    def forward(self, batch_dict):
        ...
        if self.use_pnp3d:
            ...
            # 提取点坐标
            roi_grid_xyz = batch_dict.get('roi_grid_xyz', None)
            if roi_grid_xyz is None:
                # 如果没有保存坐标，可以从roi_grid_pool方法中获取
                roi_xyz, _ = self.get_global_grid_points_of_roi(
                    batch_dict['rois'], grid_size=self.pool_cfg.GRID_SIZE
                )
                roi_grid_xyz = roi_xyz.view(batch_size_rcnn, -1, 3)

            # 准备输入格式 (B, 3, N) 和 (B, C, N)
            xyz = roi_grid_xyz.transpose(1, 2).contiguous()  # [B, 3, N]
            features = pooled_features.transpose(1, 2).contiguous()  # [B, C, N]

            # 应用PnP3D增强特征
            enhanced_features = self.pnp3d_module(xyz, features)

            # 转换回原始格式 (BxN, 6x6x6, C)
            pooled_features = enhanced_features.transpose(1, 2).contiguous()
