...
class VoxelRCNNHead(RoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        if self.use_pnp3d:
            from ...models.pnp3d.pnp3d import PnP3D

            self.pnp3d_module = PnP3D(
                input_features_dim=c_out
            )
    ...
    def forward(self, batch_dict):
        ...
        if self.use_pnp3d:
            ...
            # 应用标准版PnP3D增强特征（注意k参数需要从配置中获取）
            k = self.model_cfg.PNP3D_CONFIG.get('K', 20)  # 默认使用20个邻居
            enhanced_features = self.pnp3d_module(xyz, features, k)
